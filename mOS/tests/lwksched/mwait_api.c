...
#define MS_TO_SEC(msec) ((msec) / 1000)
...
{
	long rc = 0;
	unsigned long remainder_ms, response_time = (unsigned long)arg;
	struct timespec sleeptime;

	if (response_time >= SEC_TO_MS(1)) {
		sleeptime.tv_sec = MS_TO_SEC(response_time);
		remainder_ms = response_time % (SEC_TO_MS(sleeptime.tv_sec));
		sleeptime.tv_nsec = MS_TO_NS(remainder_ms);
	} else {
		sleeptime.tv_sec = 0;
		sleeptime.tv_nsec = MS_TO_NS(response_time);
	}
	log_msg(LOG_DEBUG,
	...
	do {
		rc = nanosleep(&sleeptime, &sleeptime);
	} while ((rc == -1) && (errno == EINTR));
	if (rc)
		log_msg(LOG_ERR,
		 " Failure executing nanosleep. errno=%d", errno);
...
int main(int argc, char **argv)
{
	...
	test_num++;
	rc_local = 0;
	accelerator_time = 500; /* 'run' 500 msecs before responding */
	mwait_timeout = accelerator_time/10; /* api timeout 50 ms */
	previous_value = monitored_field;
	log_msg(LOG_INFO, "Starting test %d", test_num);
