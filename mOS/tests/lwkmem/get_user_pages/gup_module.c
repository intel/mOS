			 ...
			 size_t arg_struct_len, loff_t * off)
{
	...
	unsigned long gup_flags = FOLL_WRITE;

	struct {
		void *src;
		void *dst;
		unsigned long len;
		int split_huge_pages;
		int verbose;
	} args;
	...
	*off = *off + arg_struct_len;
	...
	/* Map the src buffer */
	rc = get_user_pages((unsigned long)args.src, num_pages, gup_flags,
			    src_pages, src_vmas);
	if (rc != num_pages) {
	...
	rc = get_user_pages((unsigned long)args.dst, num_pages, gup_flags,
			    dst_pages, dst_vmas);
	if (rc != num_pages) {
