...
class Yod(Base):
    ...
    def test_dbss(self):
        yod(self, './maptest', '--verbose', '--type', 'dbss')
        # dbss region is always faulted in during ELF loading, but this should not fail in any case.
        yod(self, '--pagefault', 'dbss:onefault', './maptest', '--verbose', '--type', 'dbss')

    def test_heap(self):
        # Launch a heap-manipulating test app and verify the heap works
        def subtest(n, size):
            with self.subTest(n=n, size=size):
                yod(self, './maptest', '--verbose', '--type', 'heap', '--num', n, '--size', size)
                yod(self, '--pagefault', 'heap:onefault', './maptest', '--verbose', '--type', 'heap', '--num', n, '--size', size)
    ...
    def test_mremap(self):
        sizes = [ 20 * KiB, 64 * KiB, 612 * KiB,
                  1 * MiB, 1.5 * MiB, 2 * MiB, 611 * MiB,
                  1 * GiB, 1.5 * GiB, (1.5 * GiB) + (4 * KiB) ]
        scales = [ 2, 3 ]
        pagefault = [ 'all:nofault', 'all:onefault' ]
        maxpage = [ 'all:1g', 'all:2m', 'all:4k' ]
        ...
        for sz in sizes:
            for factor in scales:
                for fault in pagefault:
                    for maxpg in maxpage:
                        yod(self, '--pagefault', fault, '--maxpage', maxpg, './mremap', '-s', sz, '-f', factor, *v)
        ...
        def subtest(clrlen=None):
            with self.subTest():
                if clrlen is not None:
                    ...
                    yod(self, '--pagefault', 'heap:onefault', '--opt', 'lwkmem-brk-clear-len={}'.format(clrlen), './heaptest', '--clear-len', clrlen)
                else:
                    yod(self, './heaptest')
                    yod(self, '--pagefault', 'heap:onefault', './heaptest')
        subtest()
        subtest(clrlen=-1)
        for n in range(4):
    ...
    def test_aligned_mmap(self):
        for size in [1, 4*KiB, 256*KiB, 2*MiB, 64*MiB, 1*GiB, 2*GiB]:
            yod(self, './alignmenttest', '--verbose', '--size', size, '--iterations', 3)
    ...
    def test_fork(self):
        if ARGS.verbosity == 2:
            ...
            v = [ ]
        for map_size in [ 16*KiB, 16*MiB, 2*GiB ]:
            yod(self, './fork', '-s', map_size, *v)
