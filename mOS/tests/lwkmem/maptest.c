...
unsigned long global_data = 0xaaaacccc;
...
enum mapping_type { ANONYMOUS, DBSS, HEAP, STACK, TEXT };
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < iterations && rc == 0; i++) {
		if (type == HEAP)
			...
			head = malloc(sizeof(*head));
			if (head) {
				/* Initialized global var in .data */
				head->data = (unsigned *) &global_data;
				head->size = sizeof(global_data);
				head->tested = 0;
				head->next = malloc(sizeof(*head));
				num_allocations = 1;
				if (head->next) {
					/* Uninitialized static var in .bss */
					head->next->data =
							(unsigned *) &bss_data;
					head->next->size = sizeof(bss_data);
					head->next->tested = 0;
					head->next->next = NULL;
					num_allocations++;
				}
		    ...
		    unsigned long num_mappings)
{
	...
	while (fgets(buffer, sizeof(buffer), fptr) != NULL) {
		...
		/* Verify that .data/.bss mapping is marked as LWK memory. */
		if (type == DBSS && strstr(buffer, "[dbss]") &&
		    !strstr(buffer, "LWK")) {
			printf("(E) .data/.bss mapping is not LWK memory: %s",
			       buffer);
			rc = -1;
			goto out;
		}
	...
	/* Check if all mappings were verified. */
	for (node = head; node != NULL; node = node->next) {
		if (node->tested == 0) {
			printf("Failed to locate %p [%ld]!\n",
				node->data, node->size);
			rc = -EINVAL;
		}
	}
