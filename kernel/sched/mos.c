...
pick_next_task_mos(struct rq *rq, struct task_struct *prev, struct rq_flags *rf)
{
...
select_task_rq_mos(struct task_struct *p, int cpu, int sd_flag, int flags)
{
	...
	if (likely((cpumask_intersects(&p->cpus_allowed,
		...
		/* Primary wakeup path */
		if (likely(cpumask_test_cpu(ncpu, &p->cpus_allowed))) {
...
int mos_select_cpu_candidate(struct task_struct *p, int cpu)
{
	...
	if (likely(!cpumask_test_cpu(cpu, &p->cpus_allowed))) {
		 ...
		 */
		if (likely(cpumask_subset(&p->cpus_allowed,
				  ...
				  const struct cpumask *cpumask)
{
	...
	if (alloc_cpumask_var(&new_mask, GFP_KERNEL)) {
		...
		if (!(cpumask_intersects(new_mask, cpu_online_mask))) {
			...
			cpumask_andnot(new_mask, &p->cpus_allowed, lwkcpus);
			if (!(cpumask_intersects(new_mask, cpu_online_mask)))
