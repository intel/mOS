	...
	if (is_lwkmem_enabled(current) && !is_lwkvmr_disabled(LWK_VMR_HEAP)) {
		...
		if (next && next->vm_start <= oldbrk) {
			...
			if (newbrk > next->vm_end) {
				...
				if (next->vm_next &&
				    (newbrk + PAGE_SIZE >
				    vm_start_gap(next->vm_next)))
					goto out;
				if (do_brk_flags(next->vm_end,
					newbrk - next->vm_end,
			...
			if (do_brk_flags(oldbrk, newbrk-oldbrk,
				VM_LWK | VM_LWK_HEAP | VM_LWK_EXTRA, &uf) < 0)
				goto out;
			next = find_vma(mm, oldbrk);
		}
...
static int do_brk_flags(unsigned long addr, unsigned long len, unsigned long flags, struct list_head *uf)
{
	 ...
	 */
	vm_lwk_flags = VM_LWK | VM_LWK_HEAP | VM_LWK_DBSS | VM_LWK_EXTRA;
