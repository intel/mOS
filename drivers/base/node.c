			...
			struct device_attribute *attr, char *buf)
{
	...
	n += sprintf(buf + n,
			...
			,
		       ...
		       nid, lwk_only ? 0 :
				K(sreclaimable + node_page_state(pgdat,
						NR_KERNEL_MISC_RECLAIMABLE)),
		       nid, lwk_only ? 0 : K(sreclaimable + sunreclaimable),
		       nid, lwk_only ? 0 : K(sreclaimable),
		       nid, lwk_only ? 0 : K(sunreclaimable)
#ifdef CONFIG_TRANSPARENT_HUGEPAGE
		       ,
		       nid, lwk_only ? 0 :
